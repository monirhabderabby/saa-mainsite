...
const MarkAsSentReferenceModal = dynamic(
  () => import("./_components/mark-as-sent-reference-modal"),
  {
    ssr: false,
  },
);
...
const ViewUpdateSheetModal = ({
  ...
  const onMarkedAsSend = (referenceLink: string) => {
    startTransition(async () => {
      const res = await markAsSent(data.id, referenceLink); // pass the link to your server action
      if (!res.success) {
        toast.error(res.message);
        // Re-open the reference modal so they can retry
        return;
      }
      ...
      <AlertDialog open={open} onOpenChange={setOpen}>
        ...
        <AlertDialogContent className="p-0 gap-0">
          ...
          <ScrollArea className="max-h-[70vh] ">
            ...
            {data.sendAt && (
              <p className="p-5 pt-0 text-[14px] text-muted-foreground">
